        ...
        <div className={`flex ${role === 'user' ? 'justify-end' : 'justify-start'} group mb-4`}>
            <div className={`relative max-w-[90%] p-4 rounded-2xl text-xs leading-relaxed transition-all ${role === 'user'
                ...
                <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2 opacity-40 text-[9px] uppercase font-black tracking-widest !text-slate-400">
                        {role === 'user' ? <User size={12} /> : <Bot size={12} className="!text-purple-400" />}
                        <span className="!text-slate-400">{role === 'assistant' ? 'Dwight' : role}</span>
                    </div>
                ...
                {/* Content - Force dark mode styling for chat panel */}
                <div className="markdown-content markdown-content-dark font-medium overflow-x-auto">
